...
def train_one_epoch(model: torch.nn.Module, args, train_config,
    ...
    for step, ((WeakPoints, StrongPoints), targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        StrongPoints, WeakPoints = StrongPoints.to(device, non_blocking=True), WeakPoints.to(device, non_blocking=True)
        targets = targets.to(device, non_blocking=True)

        with torch.no_grad():             
            # pseudo-label with ema model
            probs_ema = F.softmax(model_ema.ema(WeakPoints),dim=-1)
        ...
        with amp_autocast():    
            if args.mask:
                logits, loss_mim, loss_align, mask = model(StrongPoints, mask=True)
            else:
                logits = model(StrongPoints)
            ...
            if args.mask:
                ...
                loss = loss_st + loss_mim + train_config['w_align'] * loss_align
            else:
