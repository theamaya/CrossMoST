...
def test_zeroshot_3d_core(test_loader, model, tokenizer, args=None):
    ...
    progress = ProgressMeter(
        len(test_loader),
        [batch_time, top1, top5, top1_img, top5_img],
        prefix='Test: ')
    ...
    with torch.no_grad():
        ...
        for i, ((img, pc), target) in enumerate(test_loader):
        # for i, ((_, pc), target) in enumerate(test_loader):
            ...
            pc_features = model(img, pc, val=True)['pc_embed']
            pc_features = pc_features / pc_features.norm(dim=-1, keepdim=True)

            img_features = model(img, pc, val=True)['image_embed']
    ...
    return {'acc1': top1.avg, 'acc5': top5.avg, 'acc1_img': top1_img.avg, 'acc5_img': top5_img.avg}
